...
- IFAC WC 2026 后续增加公开 HAL 记录页 `https://hal.science/hal-05673211/`；其 `Paper Link` 紧接 `IFAC WC | 2026` 元信息，并同步显示于 Publications 对应条目。本地手稿仍不公开。
- Education 卡片为 University of Lorraine/CRAN、Northeastern University 与 Guizhou University 增加对应标志；原件及网页副本映射见 [Education 学校标志材料](education-logo-materials.md)。
- 个人资料卡在 Email 与 LinkedIn 下方增加 GitHub 入口，指向网站仓库所属账号 `https://github.com/mengshengya`。
- 四个标志网页副本均在 Chrome 中成功加载并核对原始尺寸；1440 × 1100 桌面布局将标志置于学校信息右侧，500 × 900 窄屏布局将标志换行至学校信息下方，两种视口均无页面横向溢出或浏览器错误。
- Publications 中已有真实公开入口的条目显示 `Project Details` 和/或 `Paper Link` 按钮；未提供可靠链接的论文不添加占位入口。CEP 论文提供两种入口，IFAC WC 会议论文仅链接项目页。
